...
}
{
  ...
  // Automatically detect the origin of the current page
  var LOCAL_AUTO_DETECT_ORIGIN = window.location.origin;

  // Define a list of allowed origins, initially with 'null' (for local file access)
  // Add more trusted origins
  var HOSTED_VIEWER_ORIGINS = ["null"];

  // Continue to auto-detect local origin
  HOSTED_VIEWER_ORIGINS.push(LOCAL_AUTO_DETECT_ORIGIN);

  // Function to validate the PDF file URL
  var validateFileURL = function (file) {
    ...
    try {
      ...
      // Allow loading files from trusted origins
      if (
        HOSTED_VIEWER_ORIGINS.includes(viewerOrigin) ||
        HOSTED_VIEWER_ORIGINS.includes(fileOrigin)
      ) {
        return; // Proceed if file origin or viewer origin is trusted
      }

      throw new Error("File origin does not match viewer's origin");
    } catch (ex) {
